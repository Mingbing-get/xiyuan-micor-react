...
class ShowStatistic extends React.Component {
    ...
    componentDidMount(){
        ...
        const data = this.props.updFact.getData();
        this.setState({
          data,
        });
        if (data && data.cpdata){
            this.myChart.setOption(this.optionAdaptor(data.cpdata.option));
        }
    };
    componentDidUpdate(){
        if (this.state.data && this.state.data.cpdata){
            this.myChart.clear();
            this.myChart.setOption(this.optionAdaptor(this.state.data.cpdata.option));
        }
    ...
    render() {
        let style = {};
        if (this.state.data && this.state.data.cpdata){
            style = this.state.data.cpdata.style || {};
        }
        return (
