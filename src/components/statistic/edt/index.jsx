...
class EdtStatistic extends React.Component {
    ...
    propsChange(data){
      if (data && data.cpdata){
        this.setState({
          style:data.cpdata.style?{...data.cpdata.style}:{},
          option:data.cpdata.option?{...data.cpdata.option}:{},
          xAxisData:data.cpdata.option && data.cpdata.option.xAxis && data.cpdata.option.xAxis.data
        });
      }
    }
    //更新最外层容器的样式
    ...
    updateOption = (option)=>{
        ...
        this.fangdou = setTimeout(()=>{
            this.updateData({cpdata:{...this.state.edtData.cpdata ,option}});
        },1000);
    };
    updateTitle = (e)=>{
        if (e.target.value){
